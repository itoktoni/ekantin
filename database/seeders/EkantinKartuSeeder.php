...
class EkantinKartuSeeder extends Seeder
{
    ...
    {
        $kartu = [
            // [barcode, email siswa, email ortu, nis, kelas, saldo, limit]
        ...
        foreach ($kartu as [$barcode, $emailSiswa, $emailOrtu, $nis, $kelas, $saldo, $limit]) {
            $siswa = User::where('email', $emailSiswa)->firstOrFail();
            $ortu = $emailOrtu ? User::where('email', $emailOrtu)->firstOrFail() : null;
            Kartu::firstOrCreate(['kartu_barcode' => $barcode], [
                'kartu_id_user' => $siswa->id,
