...
@php
    ...
    $namaPengguna = $trx->hasKartu?->hasUser?->name ?? 'Walk-in ('.ucfirst($metode).')';
...
<x-layouts::app>
    <x-breadcrumb :items="[['url' => route('kasir.pos'), 'label' => 'Kasir POS'], ['url' => '', 'label' => 'Struk #'.$trx->transaksi_id]]" class="no-print" />

    <div class="content mt-4 lg:mt-0 flex flex-col items-center">
        <div class="no-print w-full max-w-[380px] flex flex-wrap gap-2 mb-3">
            <a href="{{ route('kasir.pos') }}" class="inline-flex items-center gap-1 h-8 px-3 text-xs font-semibold rounded-lg border border-outline-variant">← Kasir</a>
            ...
            <a href="{{ route('transaksi.getUpdate', ['id'=>$trx->transaksi_id]) }}" class="inline-flex items-center h-8 px-3 text-xs rounded-lg border">Detail</a>
        </div>
        ...
        <div class="receipt-58mm bg-white text-black">
            <div class="r-head">
                ...
                <div class="r-sub">Kantin Digital • Cashless</div>
                <div class="r-addr">Jl. Pendidikan No.1 — (021) 000-0000</div>
            </div>
            <div class="r-line">--------------------------------</div>
            <div class="r-meta">
                <div><span>No</span><b>#{{ str_pad($trx->transaksi_id,6,'0',STR_PAD_LEFT) }}</b></div>
                <div><span>Tgl</span><b>{{ formatDate($trx->created_at,'d/m/y H:i') }}</b></div>
                <div><span>Kasir</span><b class="r-trunc">{{ $kasirNama }}</b></div>
                <div><span>Jenis</span><b>{{ $trx->transaksi_jenis }} / {{ $trx->transaksi_status }}</b></div>
            ...
            <div class="r-items">
                ...
                @foreach($items as $i)
                <div class="r-item">
                    <div class="r-item-name">{{ $i->item_nama }}</div>
            ...
            <div class="r-sum">
                ...
                @else
                <div class="r-sum-pay"><span>TOTAL</span><span>{{ formatAngka((int)$trx->transaksi_total,'Rp') }}</span></div>
                @endif
            ...
            <div class="r-line">--------------------------------</div>
            <div class="r-lunas"><span>L U N A S</span></div>
            @if($trx->transaksi_status!=='berhasil')<div class="r-status">Status: {{ $trx->transaksi_status }}</div>@endif
            ...
            <div class="r-qr">
                ...
                <div class="r-qr-text">{{ $qrValue }}</div>
            </div>

            <div class="r-line">--------------------------------</div>
            <div class="r-foot">
                <div>Terima kasih — simpan untuk ambil di gerai</div>
                <div class="r-foot-mono">cetak {{ formatDate(now(),'d/m/y H:i:s') }}</div>
                <div class="r-foot-small">ECS 58mm • foto jika pudar</div>
            </div>
            <div class="r-cut">✄ - - - - - - - - - - - - - - - - - -</div>
        </div>
        </div>

        <div class="no-print text-[10px] text-on-surface-variant mt-2">Preview 58mm — Ctrl+P pilih Paper 58mm, tanpa header/footer.</div>
    </div>
    ...
    <script>
        document.getElementById('btnBluetooth')?.addEventListener('click', () => {
            ...
            const lines=[];
            lines.push({text:'E-KANTIN SEKOLAH',style:'large',align:'center'});
            ...
            lines.push({text:'',style:'normal'});lines.push({text:'Terima kasih!',style:'center'});
